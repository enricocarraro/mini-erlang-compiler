...
public class ListMatching extends Expression {
  ...
  public ListMatching(AltList lhs, Expression rhs) {
    ...
    headMatch =
        lhs.head instanceof AltList
            ? new ListMatching(
                (AltList) lhs.head, new FunctionCall("hd", new ExpressionSequence(new Variable("tmp" + currentTemporaryLabel), null)))
            : new Match(lhs.head, new FunctionCall("hd", new ExpressionSequence(new Variable("tmp" + currentTemporaryLabel), null)));
    if (lhs.tail != null && lhs.tail instanceof AltList) {
      tailMatch =
          new ListMatching(
              (AltList) lhs.tail, new FunctionCall("tl", new ExpressionSequence(new Variable("tmp" + currentTemporaryLabel), null)));

    } else {
      tailMatch =
          new Match(
              lhs.tail != null ? lhs.tail : new List(""),
              new FunctionCall("tl", new ExpressionSequence(new Variable("tmp" + currentTemporaryLabel), null)));
    }
  ...
  public long destructDependencies(Manager manager, Node caller) {
    manager.dumpln("\t; destructDependencies " + this.getClass().getName() + "(" + label + ")  caller label (" + caller.label + ")");
    ...
    if (rhs != caller) {
      if (rhs.label >= maxParentDep) {
        manager.dumpln("\t; rhs destruct " + this.getClass().getName() + "(" + rhs.label + ") max:" + maxParentDep);
        maxParentDep = Math.max(rhs.destruct(manager, this), maxParentDep);
      }
      if (headMatch != caller) {
        if (headMatch.label > maxParentDep) {
          manager.dumpln("\t; headMatch destruct " + this.getClass().getName() + "(" + headMatch.label + ") max:" + maxParentDep);
          maxParentDep = Math.max(headMatch.destruct(manager, this), maxParentDep);
        }
        if (tailMatch != caller && tailMatch.label > maxParentDep) {
          manager.dumpln("\t; tailMatch destruct " + this.getClass().getName() + "(" + tailMatch.label + ") max:" + maxParentDep);
          maxParentDep = Math.max(tailMatch.destruct(manager, this), maxParentDep);
        } else {
          manager.dumpln("\t; tailMatch == caller " + this.getClass().getName() + "(" + tailMatch.label + ") max:" + maxParentDep);
        }
      }
    }
    return maxParentDep;
