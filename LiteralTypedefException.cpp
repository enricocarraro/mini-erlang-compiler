...
typedef struct Literal
{
	...
	std::list<Literal> getList() const
	{
		debug("getList");
		if (type != List)
			throw std::invalid_argument("Type error: not a list.");
	...
	bool operator==(const Literal &rhs) const
	{
		...
		case Undefined:
			throw std::invalid_argument("Comparison between undefined variables is impossible.");
		}
	...
	bool operator<(const Literal &rhs) const
	{
		...
		case List:
			return getList() < rhs.getList();
		case Function:
			throw std::invalid_argument("Comparison between functions is not supported.");
		case Atom:
			throw std::invalid_argument("Comparison between atoms is not supported.");
		case Boolean:
			return getBoolean() < rhs.getBoolean();
		case Undefined:
			throw std::invalid_argument("Comparison between undefined variables is impossible.");
	...
	Literal listHead()
	{
		...
		if ((type != List) || !ptr)
		{
			if (!ptr)
				...
				throw std::invalid_argument("head error type");
		}
		std::list<Literal> *ptrl = (std::list<Literal> *)ptr;
		if (ptrl->empty())
		{
			throw std::invalid_argument("head error");
		}
	...
	Literal operator+(Literal const &a)
	{
		...
		if (!(isNumber() && a.isNumber()))
			throw std::invalid_argument("Invalid sum between incompatible terms.");
	...
	Literal operator*(Literal const &a)
	{
		debug("operator*");
		if (!(isNumber() && a.isNumber()))
			throw std::invalid_argument("Invalid product between incompatible terms.");
	...
	Literal operator/(Literal const &a)
	{
		debug("operator/");
		if (!(isNumber() && a.isNumber()))
			throw std::invalid_argument("Invalid division between incompatible terms.");
	...
	Literal operator%(Literal const &a)
	{
		debug("operator%");
		if (!(type != Integer && a.type != Integer))
			throw std::invalid_argument("Invalid arithmetic expression, modulo accepts only integers.");
		...
		throw std::invalid_argument("Invalid modulo between incompatible terms.");
	}
	...
	Literal negative() const
	{
		...
		throw std::invalid_argument("Cannot change sign to a non-numerical value.");
	}
...
Literal start(Literal arg)
{
	...
	{ // fun (line 8) 
		...
		if (Num != Literal(42))
		{
			throw std::invalid_argument("Bad Match");
		}
		Literal OtherUndef;
